...
private:
	Circulo(float x, float y, float width, float height, ShaderProgram &program);

public:
	// Quads can only be created inside an OpenGL context
	static Circulo *createCirculo(float x, float y, float width, float height, ShaderProgram &program);
